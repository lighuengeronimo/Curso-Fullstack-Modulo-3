...
var app = new Vue({
  el: '#app',
  data:{
    scheduleVue:[{ month:"September",collapseId:"sepSchedule",getId:"#sepSchedule",monthSchedule:[]},
                 { month:"October",collapseId:"octSchedule",getId:"#octSchedule",monthSchedule:[]}]
  }
});
...
function createSchedule() {
  ...
  filterMonth("september",cont);
  filterMonth("october",cont);

  function filterMonth(monthParam){
    app.scheduleVue[cont].monthSchedule = 
    schedule.filter(schedule => checkedBoxesTeams.includes(schedule.team1 || schedule.team2) 
                            && checkedBoxesTime.includes(schedule.time)
