...
var app = new Vue({
  ...
  methods: {
    fillLandscapeMap(school,time,adress,url) {
      document.getElementById("mapLandscape").innerHTML=
      '<p class="textInfo textInfoLeft" > '+ school + 
      '</p><p class="textInfo textInfoRight">'+ time +
      '</p><p>'+ adress + 
      '</p><iframe src="'+ url +'"></iframe>';
    }
...
for (i=0;i<schedule.length;i++){
  ...
  while(j<locations.length && !isFound){
    schedule[i].school==locations[j].school ? 
